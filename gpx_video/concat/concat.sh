...
origin_video=../../gopro/miaofengshan.gpx.mp4
clip_video=./clip000.mp4
...
## cut
ffmpeg -y -hide_banner -loglevel error -ss 20 -to 30 -i $origin_video -c copy $clip_video

ffprobe -v error -select_streams v -show_entries stream=width,height -of csv=p=0:s=x $clip_video

## scale
ffmpeg -y -hide_banner -loglevel error -i $clip_video -vf "scale=$width:$height:force_original_aspect_ratio=decrease,pad=$width:$height:-1:-1:color=black" -r $fps -an -c:v libx264 -preset veryfast $clip_video.scale.mp4
...
#
# ffmpeg -y -hide_banner -loglevel info -f concat -safe 0 -i c.txt -c copy c.mp4
# ffmpeg -y -hide_banner -loglevel info -f concat -safe 0 -i c.txt -c:v libx264 -c:a aac -preset veryfast c.mp4
